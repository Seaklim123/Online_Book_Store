        ...
        <> 
            ...
            <div className="pt-20 min-h-screen bg-[#f5eadf] pb-10">
                <div className="max-w-3xl mx-auto p-8 bg-white shadow-lg rounded-xl">
                    <h1 className="text-3xl font-extrabold mb-8 text-gray-800">{t('order_history')}</h1>
                    ...
                    {ordersList.length > 0 ? (
                        <div className="grid gap-6">
                            {ordersList.map((order) => (
                                ...
                                >
                                    <div>
                                        <p className="text-sm text-gray-500">{t('order_number')} {order.id}</p>
                        ...
                        <div className="bg-white p-10 text-center rounded-lg shadow">
                            ...
                            <Link href="/customer/books" className="inline-block mt-4 bg-[#bda081] text-white px-6 py-2 rounded">{t('start_shopping')}</Link>
                        </div>
                    )}
                </div>
            </div>
                    ...
                    return (
                        <div className="p-0 bg-white rounded-xl overflow-hidden">
                            <div className="pt-4 pr-4 pl-4 pb-2 bg-[#e5d3bf] border-b flex justify-between items-center">
                                <div>
                                    <h2 className="text-xl font-bold uppercase tracking-tight">{t('invoice')} #{selectedOrder.id}</h2>
                                    <p className="text-xs text-gray-500 font-medium uppercase"> [{t('status')}: {selectedOrder.status}]</p>
                                </div>
                            ...
                            {selectedOrder.status === 'cancelled' && (
                                <div className="mx-6 mt-4 p-3 bg-red-50 border-l-4 border-red-500 text-red-700 text-sm">
                                    <strong>{t('reason')}:</strong> {selectedOrder.cancel_reason || 'N/A'}
                                </div>
                            )}
                            ...
                            <div className="p-6 bg-gray-50 border-t grid grid-cols-2 gap-4">
                                <div className="text-xs text-gray-500">
                                    <p className="font-bold mb-1">{t('delivery_to')}: {selectedOrder.shipping_address}</p>
                                    <p className="mt-1 font-bold">{t('customer_contact')}: {selectedOrder.phone_number}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs text-gray-400 uppercase">{t('total_paid')}</p>
